...
package org.hyperledger.fabric.samples.nftpriv;
...
@Default
public final class NftPriv implements ContractInterface {
    ...
    public Asset CreateAsset(final Context ctx) {
        ...
        int itemId = Integer.parseInt(new String(transientMap.get("itemId"), StandardCharsets.UTF_8));
        String color = new String(transientMap.get("color"), StandardCharsets.UTF_8);
        int size = Integer.parseInt(new String(transientMap.get("size"), StandardCharsets.UTF_8));
        String owner = new String(transientMap.get("owner"), StandardCharsets.UTF_8);

        Asset asset = new Asset(itemId, color, size, owner);
        String assetJSON = genson.serialize(asset);
        stub.putPrivateData("assetCollection", Integer.toString(itemId), assetJSON);
        return asset;
    }
     ...
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public boolean TransferFrom(final Context ctx) {
        String submittingOrg = ctx.getClientIdentity().getMSPID();
        ChaincodeStub stub = ctx.getStub();

        Map<String, byte[]> transientMap = stub.getTransient();
        int tokenId = Integer.parseInt(new String(transientMap.get("itemId"), StandardCharsets.UTF_8));
        String from = new String(transientMap.get("from"), StandardCharsets.UTF_8);
        String to = new String(transientMap.get("to"), StandardCharsets.UTF_8);
        ...
        if (!owner.equalsIgnoreCase(from)) {
            throw new ChaincodeException("The from (" + from + ") is not the current owner (" + owner + ").", "The from (" + from + ") is not the current owner (" + owner + ").");
        }
        ...
        if(!submittingOrg.equalsIgnoreCase(from)) {
            throw new ChaincodeException(submittingOrg + " tried to transfer asset (" + tokenId + ") which it does " +
                    "not own. Owner is: " + owner, submittingOrg + " tried to transfer asset (" + tokenId + ") which it does " +
        ...
        stub.putPrivateData("assetCollection", Integer.toString(tokenId), assetJSON);
    ...
    public Asset ReadAsset(final Context ctx, final int assetID) {
        ChaincodeStub stub = ctx.getStub();
        String assetJSON = stub.getPrivateDataUTF8("assetCollection", Integer.toString(assetID));
