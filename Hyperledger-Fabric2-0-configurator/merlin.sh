...
packCC(){
  ...
  for chaincode in "${ccodes[@]}"
		do
			...
			./gradlew clean build shadowJar -x checkstyleMain
			if [ $? -eq 1 ]; then
		...
		do
			...
			peer lifecycle chaincode package ${chaincode}.tar.gz --path ${CC_SRC_PATH}/${chaincode}/build/libs --lang java --label fabnft_${VERSION} > ${OUTPUTDEV}
			if [ $? -eq 1 ]; then
	...
	if [ ${#ccodes[@]} -eq 0 ]; then
		...
		for (( org = 1; org <= $NO_ORGANIZATIONS; org++ )); do
			...
			do
				...
				if [ "${chaincode}" == "fabnftpub" ]; then
					peer lifecycle chaincode approveformyorg ${ORDERERS} --channelID ${CHANNEL_ID} --name fabnft --version ${VERSION} --package-id $(sed -n "/fabnft_1/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt) --sequence ${VERSION} --waitForEvent > ${OUTPUTDEV}
				else
					peer lifecycle chaincode approveformyorg ${ORDERERS} --channelID ${CHANNEL_ID} --name fabnft --version ${VERSION} --package-id $(sed -n "/fabnft_1/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt) --sequence ${VERSION} --waitForEvent > ${OUTPUTDEV} --collections-config ./collections_config${NO_ORGANIZATIONS}.json
				fi
				if [ $? -eq 1 ]; then
  ...
  echo -e "${PURPLE}  \"88,d88 888 888  \"YeeP\"  \"88,e8' 888 8b     \"88,d88  \"88 88\"  888 888 888 888 888 888 888  888"
	...
	if [ ${#ccodes[@]} -eq 0 ]; then
		...
		do
			...
			if [ "${chaincode}" == "fabnftpub" ]; then 
				peer lifecycle chaincode checkcommitreadiness --channelID ${CHANNEL_ID} --name fabnft --version ${VERSION} --sequence ${VERSION} --output json > ${OUTPUTDEV}
			else
				peer lifecycle chaincode checkcommitreadiness --channelID ${CHANNEL_ID} --name fabnft --version ${VERSION} --sequence ${VERSION} --output json > ${OUTPUTDEV} --collections-config ./collections_config${NO_ORGANIZATIONS}.json
			fi
			if [ $? -eq 1 ]; then
...
commitCC(){
	...
	if [ ${#ccodes[@]} -eq 0 ]; then
		...
		do
			...
			if [ "${chaincode}" == "fabnftpub" ]; then 
				peer lifecycle chaincode commit ${ORDERERS} --channelID ${CHANNEL_ID} --name fabnft $PEER_CON_PARAMS --version ${VERSION} --sequence ${VERSION} > ${OUTPUTDEV}
			else
				peer lifecycle chaincode commit ${ORDERERS} --channelID ${CHANNEL_ID} --name fabnft $PEER_CON_PARAMS --version ${VERSION} --sequence ${VERSION} > ${OUTPUTDEV} --collections-config ./collections_config${NO_ORGANIZATIONS}.json
			fi
			if [ $? -eq 1 ]; then
		...
		for chaincode in "${ccodes[@]}"
		do
			peer lifecycle chaincode querycommitted --channelID ${CHANNEL_ID} --name fabnft > ${OUTPUTDEV}
		done
		echo -e "\n${LIGHTCYAN} =<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<=<="
	fi
}
...
if [ "${chaincode}" == "fabnftpub" ]; then
	./runfabnftpub${NO_ORGANIZATIONS}.sh #| grep 'Submitted:'
else
	./runfabnftpriv${NO_ORGANIZATIONS}.sh #| grep 'Submitted:'
fi

printf '\7'

cd /mnt/c/Users/soelm/Desktop/Repositories/FabricBesuBenchmark/Hyperledger-Fabric2-0-configurator
docker-compose down
