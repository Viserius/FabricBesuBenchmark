...
def generate_connection_profile(_network_config: NetworkConfiguration,
    ...
    for orderer in orderer_list:
        ordes.update({
            orderer: {
                "url": f"grpcs://localhost:{_network_config.orderer_defport+1000*i}",
                "grpcOptions": {
                    "ssl-target-name-override": orderer
                },
                "tlsCACerts": {
                    "path": f"../Hyperledger-Fabric2-0-configurator/crypto-config/ordererOrganizations/{_domain}/orderers/orderer{i+1}.{_domain}/msp/tlscacerts/tlsca.{_domain}-cert.pem"
                }
            }
        })
        i += 1
    ...
    for peer in range(_peers):
        for org in range(_orgs):
            peer_ls.update({f"peer{peer}.org{org+1}.{_domain}": {
                "url": f"grpcs://localhost:{_network_config.peer_defport + 1000 * ((_peers * org) + peer)}",
                "grpcOptions": {
                    "ssl-target-name-override": f"peer{peer}.org{org+1}.{_domain}",
                    "request-timeout": 120001
                },
                "tlsCACerts": {
                    "path": f"../Hyperledger-Fabric2-0-configurator/crypto-config/peerOrganizations/org{org+1}.{_domain}/peers/peer{peer}.org{org+1}.{_domain}/msp/tlscacerts/tlsca.org{org+1}.{_domain}-cert.pem"
                }
            }})
    ...
    for org in range(_orgs):
        ca_ls.update({
            "ca.org{}.{}".format(org+1, _domain): {
                ...
                "tlsCACerts": {
                    "path": f"../Hyperledger-Fabric2-0-configurator/crypto-config/peerOrganizations/org{org+1}.{_domain}/ca/ca.org{org+1}.{_domain}-cert.pem"
                }
            }
        })
        i += 1
