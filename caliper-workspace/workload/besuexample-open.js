...
class SimpleOpenWorkload extends WorkloadModuleBase {
    ...
    _idToPrivKey() {
        // Priv. key > 0, thus, set txIndex to at least 1
        let txToHex = (this.txIndex+1).toString(16) + "000" + this.workerIndex;
    ...
    _generateWorkload() {
        ...
        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            ...
            workload.push({
                contract: 'besuexample',
                verb: 'open',
                args: [accountId, this.roundArguments.money],
                sender: {nonce: 0, privateKey: this._idToPrivKey() },
                readOnly: false
            });
        }
        return workload;
